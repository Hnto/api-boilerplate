...
class InstallApiCommand extends BaseCommand
{
    ...
    protected function configure()
    {
        $this
            ->setName('install')
            ->setDescription('Installs the Api guid generator application')
            ->setHelp('This command allows you to do a first time install of Api guid generator');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (file_exists($this->basePath . '.env') && file_exists($this->basePath . 'app/.env')) {
            $this->style->error('Api has already been installed');
            return 1;
        }

        $this->setupProgressBar();

        /** First step -- copy .env.dist files to .env and setup project name */
        ...
        $this->buildSuccessMessage('Api has been successfully installed. ');
        $this->buildTable(
    ...
    private function copyExampleEnvToEnv()
    {
        ...
        if (copy($this->basePath . 'app/.env.dist', 'app/.env') === false) {
            $this->removeEnvFiles();
            $this->sendErrorMessage(
                'could not copy env examples to env file, install failed. Please modify the changes manually'
            );
    ...
    private function chooseEnvironment(QuestionHelper $questionHelper)
    {
        ...
        $path = $this->basePath . 'app/.env';

        if (file_put_contents($path, str_replace(
            'APP_ENV=dev', 'APP_ENV='.$environment, file_get_contents($path)
        )) === false) {
            $this->removeEnvFiles();
    ...
    private function chooseDatabasePasswords(QuestionHelper $questionHelper)
    {
        $question = new Question(
            'Please enter the password for the api username of the database (or leave empty for default "secret"):',
            'secret'
        );
        $this->style->newLine();
        ...
        $appPath = $this->basePath . 'app/.env';
        $installPath = $this->basePath . '.env';

        if (file_put_contents($appPath, str_replace(
            'DATABASE_URL=mysql://api:secret@mysql:3306/api',
            'DATABASE_URL=mysql://api:'. $userPassword .'@mysql:3306/api',
    ...
    private function setupApplication()
    {
        ...
        $process = new Process('docker exec ' . $this->projectName . self::WORKSPACE_SUFFIX . ' bin/console --no-interaction doctrine:migrations:migrate');
        $process->start();
